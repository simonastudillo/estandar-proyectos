...
export class CodeMetricsCollector {
  ...
  private getAllSourceFiles(): string[] {
    ...
    return files.filter(
      (file) =>
        file.endsWith(".ts") || file.endsWith(".tsx") || file.endsWith(".php")
    );
  }
  ...
  private detectCodeSmells(content: string, filePath: string): string[] {
    ...
    if (magicNumbers && magicNumbers.length > 5) {
      smells.push(`Magic Numbers: ${magicNumbers.length} magic numbers found`);
    }
  ...
  private evaluateQualityGate(summary: SummaryMetrics): QualityGateResult {
    ...
    if (
      summary.averageMaintainability < this.qualityThresholds.minMaintainability
    ) {
      failedCriteria.push(
    ...
    <div class="container">
        ...
        <div class="content">
            ...
            <div class="metrics-grid">
                ...
                <div class="metric-card">
                    <div class="metric-value">${summary.overallCoverage.toFixed(
                      1
                    )}%</div>
...
${qualityGate.passed ? "✅ **PASSED**" : "❌ **FAILED**"} - Score: ${
      qualityGate.score
    }/100
...
${metrics.files
  ...
  .map(
    (file) =>
      `| ${file.path.split("/").pop()} | ${file.lines} | ${file.complexity} | ${
        file.maintainabilityIndex
      } | ${file.codeSmells.length} |`
  )
  .join("\n")}
...
async function runCodeAudit(): Promise<void> {
  ...
  try {
    ...
    console.log(
      `Quality Gate: ${metrics.qualityGate.passed ? "✅ PASSED" : "❌ FAILED"}`
    );
