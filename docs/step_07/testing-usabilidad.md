...
export class UsabilityTestRunner {
  ...
  completeTask(
    ...
    const timeSpent = (new Date().getTime() - this.startTime.getTime()) / 1000;
    ...
    console.log(
      `${completed ? "✅" : "❌"} ${completed ? "Completada" : "No completada"}`
    );
  }
  ...
  generateReport(): UsabilityReport {
    ...
    const completedTasks = this.session.tasks.filter((t) => t.completed).length;
    const averageTime =
      this.session.tasks.reduce((sum, t) => sum + t.timeSpent, 0) / totalTasks;
  ...
  private generateRecommendations(): string[] {
    ...
    if (highFrustrationTasks.length > 0) {
      recommendations.push("Simplificar tareas que generan alta frustración");
    }
...
export class CognitiveLoadAnalyzer {
  ...
  private static generateRecommendations(
    ...
    if (metrics.colorVariety > 12) {
      recommendations.push("Establecer una paleta de colores más consistente");
      recommendations.push("Usar color de manera estratégica para jerarquía");
    }
